...
base_command=(
    ...
    "--window-pattern=${BASE_WINDOW_PATTERN}"
    ...
    "--core-metric-max-per-task=${BASE_CORE_METRIC_MAX_PER_TASK}"
    "--sample-every=${BASE_SAMPLE_EVERY}"
    "--save-every=${BASE_SAVE_EVERY}"
    "--model-tag=${BASE_MODEL_TAG}"
    "--run=${EXPERIMENT_ID}-base"
    "--swanlab-mode=${NANOCHAT_SWANLAB_MODE}"
    "--swanlab-project=${NANOCHAT_SWANLAB_PROJECT}"
    "--swanlab-group=${NANOCHAT_SWANLAB_GROUP}"
    "--swanlab-tags=${NANOCHAT_SWANLAB_TAGS},base"
)
"${base_command[@]}" 2>&1 | tee "${EXPERIMENT_RUNTIME_DIR}/base-train.log"
